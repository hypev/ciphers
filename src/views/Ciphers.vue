<template>
    <v-row class="mt-5">
        <!-- PLAIN TEXT INPUT -->
        <v-col cols="12" lg="4" order-lg="0" md="6" order-md="1" sm="12" order-sm="0">
            <v-card elevation="2" class="pb-3">
                <v-card-title>{{ firstTitle }}</v-card-title>
                <v-textarea outlined name="input-7-4" label="Text" class="ma-3" v-model="firstText"></v-textarea>
            </v-card>
        </v-col>

        <!-- SETTINGS -->
        <v-col cols="12" lg="4" order-lg="1" md="12" order-md="0" sm="12" order-sm="1">
            <v-card elevation="2" class="pb-3">
                <v-card-title>Settings</v-card-title>

                <!-- ENCODE / DECODE BTN -->
                <div class="d-flex align-center justify-center">
                    <v-btn text color="deep-purple lighten-3" :class="{ 'v-btn--active': encode }" @click="encode = true" v-if="algo != 'Blowfish'">
                        Encode
                    </v-btn>
                    <v-btn text color="deep-purple lighten-3" :class="{ 'v-btn--active': !encode }" @click="encode = false" v-if="algo != 'Blowfish'">
                        Decode
                    </v-btn>
                    <BlowfishAnimation :style="displayBF" :disabled="!allowAnimate" :plain-text="firstText" :cipher-text="secondText" />
                </div>

                <!-- ALGORITHM SELECT -->
                <v-select :items="algos" label="Alogrithm" solo class="mt-8 mx-3" v-model="algo"></v-select>
                ...
                <div class="mt-2 mx-3" v-if="algo == 'Ceaser'">
                    ...
                    <v-slider v-model="offset" thumb-label="always" min="0" max="25"></v-slider>
                </div>

                <!-- VIGENER/PLAYFAIR/TRANSPOSITION KEY INPUT -->
                <div class="mt-2 mx-4" v-if="algo == 'Vigener' || algo == 'Playfair' || algo == 'Transposition' || algo == 'Blowfish'">
                    <v-text-field label="Key" v-model="key"></v-text-field>

                    <!-- PLAYFAIR MATRIX -->
                    <div v-if="key && algo == 'Playfair'" class="mt-2 d-flex flex-column align-center justify-center">
                        <v-card-title>Matrix</v-card-title>
                        <div v-for="(subarray, index) in playfairMatrix" :key="index" class="d-flex my-1">
                            <div
                ...
                <div class="d-flex align-center justify-center" v-if="algo == 'RSA'">
                    <v-textarea outlined name="input-7-4" label="Public key" class="ma-3" v-model="publicKey"></v-textarea>
                    <v-textarea outlined name="input-7-4" label="Private key" class="ma-3" v-model="privateKey"></v-textarea>
                </div>

                <v-card-title v-if="algo == 'RSA'">Key length</v-card-title>

                <v-select :items="keySizes" label="Key Size" solo class="mx-3" v-model="keySize" v-if="algo == 'RSA'"></v-select>
        ...
        <v-col cols="12" lg="4" order-lg="2" md="6" order-md="2" sm="12" order-sm="2">
            <v-card elevation="2" class="pb-3">
                <v-card-title>{{ secondTitle }}</v-card-title>
                <v-textarea solo name="input-7-4" class="ma-3" :value="secondText" disabled></v-textarea>
            </v-card>
        </v-col>
    </v-row>
</template>
...
export default {
    ...
    computed: {
        ...
        secondText() {
            if (this.firstText) {
                if (this.algo == "Ceaser" && this.offset >= 0 && this.offset <= 25 && this.firstText) {
                    return Ceaser(this.firstText, this.offset * (this.encode ? 1 : -1));
                }
                if (this.algo == "Vigener" && this.key && this.firstText) {
                    if (this.encode) return Vigenere.encrypt(this.firstText, this.key).toUpperCase();
                    else return Vigenere.decrypt(this.firstText, this.key).toUpperCase();
                }
                if (this.algo == "Playfair" && this.key && this.firstText) {
                    if (this.encode) return Playfair.encrypt(this.firstText, this.key).toUpperCase();
                    else return Playfair.decrypt(this.firstText, this.key).toUpperCase();
                }
                if (this.algo == "Transposition" && this.key && this.pad && this.firstText) {
                    if (this.encode) return Transposition.encrypt(this.firstText, this.key, this.pad).toUpperCase();
                    else return Transposition.decrypt(this.firstText, this.key).toUpperCase();
                }
                ...
                if (this.algo == "RSA" && this.privateKey && this.publicKey && this.firstText) {
                    let key = RSA();
        ...
        allowAnimateRSA() {
            return this.algo == "RSA" && this.publicKey && this.privateKey && this.firstText;
        },
